...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENSY Digi Dev                  [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "digistore" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu gih Ke Digi${NC}"
    echo -e "${YELLOW}TELEGRAM : @ppgelap828${NC}"
    echo -e "${YELLOW}WHATSAPP : 6285707026043${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 10K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}©Digi Store${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer Adtya Dev ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : ppgelap828   ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : Digi Store   ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by Digi Store   ${NC}"
